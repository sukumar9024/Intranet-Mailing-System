...
$query = "SELECT * FROM emails WHERE sender_email = '$senderEmail' OR recipient_email='$senderEmail'";
...
<body>
    ...
    <div id="main">
        ...
        <h2 style="text-align:center">All Mails</h2>
        ...
        <?php while ($row = $result->fetch_assoc()): ?>
            <div class="email" onclick="toggleEmailContent(this)">
                ...
                <p>To:<?=htmlspecialchars($row["recipient_email"])?></p>
                <div class="email-content" id="emailContent<?= $row['id'] ?>">
                    <!-- Additional details or expanded content can be added here -->
                    <p><?= htmlspecialchars($row["body"]) ?></p>
                    <!-- Display attachment link if it exists -->
                    <?php if (!empty($row["attachments"])): ?>
                        <p>
                            Attachment: 
                            <a class="attachment-link" href="download_attachment.php?email=<?= $row['id'] ?>" target="_blank" onclick="downloadAttachment('<?= $row['id'] ?>', '<?= $row['attachments'] ?>')">
                                Download Attachment
                            </a>
                        </p>
                    <?php endif; ?>
    ...
    function downloadAttachment(emailId, attachmentPath) {
        // Construct the download URL
        var downloadUrl = 'download_attachment.php?email=' + emailId;
